...
jobs:
 ...
 echo "[WARN]️ Already ${EXISTING} branches for ${TASK_TYPE}"
 else
...
def check_task_complete(task_type, branch='main'):
 ...
 if all(matches):
 print(f"[OK] Task '{signature['description']}' already complete on {branch}")
 return True
...
if __name__ == '__main__':
 ...
 print("[STOP] STOP: Task already complete")
 sys.exit(1) # Signal to skip Jules invocation
 else:
 print("[OK] PROCEED: Task not complete")
 sys.exit(0)
```
...
def main():
 ...
 if age_days > 7:
 print(f"[FAIL] Base commit is {age_days} days old")
 print("[CYCLE] Fetching latest main...")
 subprocess.run(['git', 'fetch', 'origin', 'main'])
 subprocess.run(['git', 'reset', '--hard', 'origin/main'])
 print("[OK] Reset to fresh main")
 else:
 print(f"[OK] Base commit is {age_days} days old (acceptable)")
...
def main():
 ...
 print(f"[SUCCESS] Running tournament on {len(branches)} branches...")
...
jobs:
 ...
 echo "[OK] Cleanup complete. ${REMAINING} branches remaining."
```
...
echo "[SEARCH] Analyzing Jules branches..."
